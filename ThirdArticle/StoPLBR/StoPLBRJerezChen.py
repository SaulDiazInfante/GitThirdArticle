...
#
	def BEM(self):
		...
		return ubem
#
	def SSLS(self, seed=123456789, Uzero=[1,1], fn=1.0):
		h = self.Dt
		L = self.L
		ss = 1
		R = self.R
		...
		for j in np.arange(L-1):
			...
			increment = Ustar + fn*np.dot(self.b(Ustar), self.Winc)
...
#
	def LongTimeBehavior(self, seed, K=1):
		...
		for j in np.arange(K):
			...
			self.detUas[0] = self.Urk[-1]
			self.NoiseUpdate(seed,flag=0)
			self.Uas = self.SSLS(seed, self.Uas[0])
			self.detUas = self.RK(self.detUas[0])
		Uas = self.Uas
		detUas = self.detUas
		return Uas, detUas
#	
	def BoneMass(self,r,rSto,k2Sto):
		h = self.Dt
		L = self.L
		R = self.R
		k1 = self.k1
		k2 = self.k2
		self.z = np.zeros(L)
		self.zSto = np.zeros(L)
		self.z[0] = .96
		self.zSto[0] = .96
		
		Ubar = self.Ubar
		Ustk = self.SSLS(123456789, self.Uzero, fn=1.0)
		Urk = self.RK()
		den=0
		num=0
		for k in np.arange(9090):
			num = num + 0.5*(Urk[k,0]+Urk[k+1,0]) * h
			den = den + 0.5*(Urk[k,1]+Urk[k+1,1]) * h
		self.rho = num/den
		den=0
		num=0
		...
		for j in np.arange(L-1):
			...
			usj1 = Ustk[j,1]
			#
			Z = (-k1* np.max([0,uj0-Ubar[0]]) + k2 * np.max([0,uj1-Ubar[1]]))
			#Zplus = (-k1* np.max([0,ujplus0-Ubar[0]]) + k2 * np.max([0,ujplus1-Ubar[1]]))
			Zsto = (-k1Sto* np.max([0,usj0-Ubar[0]]) + k2Sto * np.max([0,usj1-Ubar[1]]))
			self.z[j+1] = self.z[j] + h * Z
			self.zSto[j+1] = self.zSto[j]+ h * Zsto
	#
	def SaveData(self):
		...
		#U1 = self.Urk[:, 0]
		#U2 = self.Urk[:, 1]
		Ustk = self.SSLS(123456789, U0, fn=0.0)
		U1 = Ustk[:, 0]
		U2 = Ustk[:, 1]
		#
		stoU1 = self.Ussls[:, 0]
